...
class UnitOfWork {
    ...
    registerManaged(entity, data, options) {
        ...
        if (data && (options?.refresh || !wrapped.__originalEntityData)) {
            ...
            wrapped.__meta.props.forEach(prop => {
                if (prop.reference === enums_1.ReferenceType.EMBEDDED && !prop.object && Utils_1.Utils.isPlainObject(data[prop.name])) {
    ...
    unsetIdentity(entity) {
        ...
        for (const prop of wrapped.__meta.bidirectionalRelations) {
            ...
            if (!Utils_1.Utils.isCollection(rel) && rel?.[inverse] && entity === entity_1.Reference.unwrapReference(rel[inverse])) {
    ...
    findNewEntities(entity, visited = new Set(), idx = 0) {
        ...
        wrapped.__schema ?? (wrapped.__schema = this.em.schema);
        this.initIdentifier(entity);
    ...
    async commitCreateChangeSets(changeSets, ctx) {
        ...
        const props = changeSets[0].meta.relations.filter(prop => {
            return (prop.reference === enums_1.ReferenceType.ONE_TO_ONE && prop.owner)
                || prop.reference === enums_1.ReferenceType.MANY_TO_ONE
                || (prop.reference === enums_1.ReferenceType.MANY_TO_MANY && prop.owner && !this.platform.usesPivotTable());
        });
